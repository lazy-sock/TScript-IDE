...
export function createDefaultServices() {
  return {
    ...
    canvas: {
      ...
      width: function () {
        if (!(this.canvas.target ? this.canvas.target : this.canvas.dom))
          return 0;
      ...
      setLineWidth: function (width) {
        let bmp = this.canvas.target ? this.canvas.target : this.canvas.dom;
      ...
      clear: function () {
        ...
        ctx.fillRect(
          0,
          0,
          (this.canvas.target ? this.canvas.target : this.canvas.dom).width,
          (this.canvas.target ? this.canvas.target : this.canvas.dom).height,
        );
      },
      ...
      paintImageSection: function (dx, dy, dw, dh, source, sx, sy, sw, sh) {
        let bmp = this.canvas.target ? this.canvas.target : this.canvas.dom;
        if (!bmp || !bmp.getContext) return;
        let ctx = bmp.getContext("2d");
        if (!source) source = this.canvas.dom;
